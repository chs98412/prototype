...
영화·시리즈·예능 콘텐츠 기록 및 게임화 플랫폼. 모바일 퍼스트 웹앱으로 시작해 Capacitor를 통해 iOS/Android 앱으로 확장한다. Go 백엔드 API + Next.js 프론트엔드로 구성된 명확한 클라이언트-서버 분리 구조를 채택한다.
...
| Cache | Upstash Redis | - | 서버리스, 무료 티어, TMDB 응답 캐싱 |
| Auth | Supabase Auth | - | Google/Kakao OAuth 내장 |
| 콘텐츠 DB | TMDB API | v3 | 글로벌 영화·시리즈·예능 데이터, 무료 |
